...
instance Y.FromJSON Mode

data AuditStep = AuditStep { run :: Text, expect :: Maybe Text }
  deriving (Generic, Show)

instance Y.FromJSON AuditStep
...
newtype Passed = Passed { unPassed :: Bool }
  deriving (Eq, Show)
...
benchmarkSucceeded benchmark outputs =
  let shouldSkip = isJust $ skip benchmark
      check =
               case (fromMaybe All (mode benchmark)) of
                 Any -> any
                 All -> all
  in shouldSkip || check (\(step, (exitCode, out, _)) -> exitCode == ExitSuccess &&
                                                         outputMatches out (expect step)) outputs
...
createReport benchmarkResults =
  let hdr = fromList ["Section", "Description", "Run", "Passed", "Output", "Error", "Skip"]
  in Csv.encodeByName hdr benchmarkResults

runScan :: FilePath -> IO (Either String Bool)
runScan spec = do
  benchmarks <- Y.decodeFileEither spec :: IO (Either Y.ParseException [Benchmark])
  case benchmarks of
    Right bs -> do
